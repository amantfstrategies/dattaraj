...
function Page({params}) {
  const pageData = MiniBusesData.find((car) => car.id === params.car);

  const currentIndex = MiniBusesData.findIndex((car) => car.id === params.car);

  const nextCarsCount = Math.min(3, MiniBusesData.length - 1);  // Show up to 3 cars, but not more than available
  const nextCars = [];
  
  for (let i = 1; i <= nextCarsCount; i++) {
    nextCars.push(MiniBusesData[(currentIndex + i) % MiniBusesData.length]);
  }
